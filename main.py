...
watermark, logo, readable text, signature, name, letters, typography, poster, illustration"""
    ...
    for attempt in range(2):
        try:
            ...
            print(f"AI Horde image generated with {result.get('generations',[{}])[0].get('model','Juggernaut XL')}.")
            return
        except Exception as exc:
            last_error = str(exc)
...
"""
    ...
    if test_only:
        ...
        print(json.dumps({"topic": topic, "music": music.name, "image": str(image), "video": str(video), "image_model": "AI Horde Juggernaut XL img2img", "video_model": "FFmpeg cinematic motion", "reference_used_as_style_input": True}, ensure_ascii=False))
        return
