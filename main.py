...
def generate_reference_guided_image(prompt: str, output: Path):
    ...
    client = Client("OmniGen2/OmniGen2", token=hf_token)

    instruction = """
Edit the FIRST image and preserve the SAME real adult human hand exactly.
This is a real hand photograph. Do NOT generate a replacement hand.

FIRST remove the existing artwork, landscape, temple, mountains, writing, signature and all old
marks from the skin while keeping the exact natural skin, palm shape, wrist, thumb, four fingers,
nails, pores, fingerprints, palm creases, lighting and camera perspective.

THEN create premium devotional Palm Art directly on that SAME real skin:
- palm facing the camera, complete wrist through all five fingertips, exactly five natural fingers
- dense handmade BLUE/INDIGO BALLPOINT-PEN drawing physically on the skin
- thousands of thin imperfect pen strokes, fine hatching, cross-hatching, contour lines and stippling
- ink follows natural palm creases and finger contours; skin texture remains visible
- at the EXACT CENTER of the palm, a LARGE, unmistakable, highly recognizable Lord Shiva / Mahadev
  figure occupying about 35-45% of the palm: recognizable face, calm eyes, third eye, jata,
  crescent moon, snake, shoulders/torso, meditative pose and trishul
- around Mahadev, densely connected miniature devotional details: Himalayan temple, bells, oil lamps,
  flowers, river/ghat, mountains, trees and tiny pilgrims
- 2-3 real blue/black ballpoint pens beside the wrist
- premium photorealistic macro editorial photograph, white/light background, realistic natural skin

The devotional figure must be the main visual subject in the center; do NOT turn this into a
mountain-only landscape. Every visible finger should contain real blue pen linework.

STRICTLY NO: tattoo, henna, mehndi, decal, sticker, printed glove, digital overlay, CGI,
3D render, vector, marker, paint, watercolor, plastic/synthetic hand, sculpture,
generic blue nails, blank fingers, extra/missing/fused/malformed fingers, cropped fingertips,
watermark, logo, readable text, signature, name or letters.

The final image must look like a real photograph of an artist who painstakingly drew
Mahadev and a dense Hindu devotional miniature world directly onto one real human palm.
""" + "\n" + prompt

    negative = (
        "tattoo, henna, mehndi, decal, sticker, printed glove, digital overlay, CGI, 3D render, "
        "vector, marker, paint, watercolor, plastic hand, synthetic hand, sculpture, text, letters, "
        "signature, watermark, logo, extra fingers, missing fingers, fused fingers, malformed hands, "
        "cropped hand, cropped fingertips, blank fingers, generic landscape-only scene, blue nails only"
    )
    ...
    local = _first_local_file(result)
    if local:
        shutil.copyfile(local, output)
    elif isinstance(result, Image.Image):
        result.save(output)
    else:
        raise RuntimeError(f"OmniGen2 returned an unsupported output: {type(result)!r} / {str(result)[:1000]}")

    if output.stat().st_size < 10000:
        raise RuntimeError("OmniGen2 returned an unexpectedly small image file.")
    ...
    print("Image generated with OmniGen2 official HF ZeroGPU edit Space.")
...
"""
    ...
    if test_only:
        ...
        print(json.dumps({"topic": topic, "music": music.name, "image": str(image), "video": str(video), "image_model": "OmniGen2 HF ZeroGPU", "video_model": "FFmpeg cinematic motion", "reference_used_as_style_input": True}, ensure_ascii=False))
        return

    fb = facebook_reel(video, title, description)
    yt = youtube_upload(video, title, description)
    print(json.dumps({"topic": topic, "facebook": fb, "youtube_video_id": yt, "music": music.name, "image_model": "OmniGen2 HF ZeroGPU", "video_model": "FFmpeg cinematic motion", "reference_used_as_style_input": True}, ensure_ascii=False))
