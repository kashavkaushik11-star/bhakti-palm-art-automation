...
def generate_reference_guided_image(prompt: str, output: Path):
    ...
    # TEST ONLY: direct text-to-image using Free.ai self-hosted Lumina Image 2.
    # No reference image is used in this test.
    generation_prompt = f"""Create a completely NEW premium photorealistic vertical devotional Palm-Art macro photograph.

MAIN SUBJECT:
One real adult human hand, palm facing directly toward the camera, centered, complete and fully visible from wrist through all five fingertips. Show exactly five natural fingers: thumb plus four fingers. Natural anatomy, realistic nails, pores, fingerprints, palm creases and skin texture. Keep the entire hand inside the frame with no cropped fingertips.

PALM ART:
The entire visible palm and fingers are densely covered with authentic handmade BLUE/INDIGO BALLPOINT-PEN drawing directly on real skin. Thousands of very thin imperfect pen strokes, fine hatching, cross-hatching, contour lines, stippling and miniature sketch details must follow the natural palm creases and finger contours. The ink is physically drawn onto the skin, not printed.

CENTERPIECE:
At the exact center of the palm, create a LARGE, unmistakable, highly recognizable Lord Shiva / Mahadev figure occupying roughly 35–45% of the palm. It must be an actual devotional figure, not a symbol or landscape: recognizable face, calm eyes, third eye, long matted jata hair, crescent moon, snake, shoulders/torso, meditative posture and a clearly drawn trishul. The face and figure must be the visual focal point.

SURROUNDING MINIATURE DEVOTIONAL WORLD:
Around Mahadev, densely connect small hand-drawn Himalayan devotional scenes: a temple, bells, oil lamps, flowers, river/ghat, mountains, trees, tiny pilgrims and sacred architectural details. Keep these secondary to the central Mahadev figure.

PHOTOGRAPHIC REALISM:
Real macro editorial photograph of a real human hand, not an illustration. Preserve natural skin pores, wrinkles and fingerprints between the blue ink strokes. Soft clean white/light studio background, realistic shadows, sharp fine ink detail. Place 2–3 real blue/black ballpoint pens beside the wrist as physical drawing tools.

STRICT NEGATIVE:
No tattoo, no henna, no mehndi, no decal, no sticker, no printed glove, no digital overlay, no CGI hand, no 3D render, no vector art, no marker, no paint, no watercolor, no plastic or synthetic skin, no sparse symbols, no blank fingers, no mountain-only composition, no landscape-only composition, no generic blue nails, no extra fingers, no missing fingers, no fused fingers, no malformed hand, no cropped hand, no watermark, no logo, no readable text, no signature, no name.

The final image must look like a real photographer photographed a real adult hand after an artist painstakingly drew a dense Hindu devotional blue ballpoint-pen artwork directly across the entire palm and fingers, with Mahadev clearly dominating the center.""" 

    url = "https://api.free.ai/v1/image/generate/"
    payload = {
        "model": "lumina-image-2",
    ...
    for attempt in range(3):
        try:
            ...
            if not r.ok:
                raise RuntimeError(f"Free.ai Lumina Image 2 failed ({r.status_code}): {r.text[:2500]}")
            data = r.json()
            image_url = data.get("output_url") or data.get("image_url") or data.get("url") or data.get("share_url")
            if not image_url:
                result = data.get("result")
                if isinstance(result, dict):
                    image_url = result.get("output_url") or result.get("image_url") or result.get("url")
            if not image_url:
                raise RuntimeError(f"Free.ai returned no image URL: {str(data)[:2500]}")
            img = requests.get(image_url, timeout=180)
            if not img.ok or len(img.content) < 10000:
                raise RuntimeError(f"Failed to download generated image ({img.status_code}, {len(img.content)} bytes)")
            ...
            print("TEST_ONLY: Lumina Image 2 direct generation completed; no reference image used.")
            return
        except Exception as exc:
            last_error = str(exc)
            print(f"Lumina Image 2 attempt {attempt + 1}/3 failed: {last_error}")
            if attempt < 2:
                time.sleep(min(8 * (attempt + 1), 20))
    raise RuntimeError(f"Lumina Image 2 generation failed: {last_error}")
...
"""
    ...
    if test_only:
        ...
        print(json.dumps({"topic": topic, "music": music.name, "image": str(image), "video": str(video), "image_model": "Free.ai Lumina Image 2", "video_model": "FFmpeg cinematic motion", "reference_used_as_style_input": False}, ensure_ascii=False))
        return

    fb = facebook_reel(video, title, description)
    yt = youtube_upload(video, title, description)
    print(json.dumps({"topic": topic, "facebook": fb, "youtube_video_id": yt, "music": music.name, "image_model": "Free.ai Lumina Image 2", "video_model": "FFmpeg cinematic motion", "reference_used_as_style_input": False}, ensure_ascii=False))
